    ...
    *,
    ...
    if models_fixture is not None:
        ...
        if not opencode_path:
            ...
            return result, failures
        version = command_result([opencode_path, "--version"], timeout_seconds=20)
        models = command_result([opencode_path, "models", provider], timeout_seconds=60)
    ...
    result["available_model_count"] = len(available)
    result["model_available"] = provider_model in available
    result["status"] = "pass" if result["model_available"] and not failures else "failed"
...
def run_opencode_smoke(
    ...
    *,
    ...
    failures: list[str] = []
    if not opencode_path:
        return {"status": "failed", "reason": "opencode binary not found"}, ["opencode binary not found"]
    ...
    with tempfile.TemporaryDirectory(prefix=f"goal-config-{role}-") as tmp:
        command = [
            ...
            tmp,
            "--title",
            f"goal-config-{role}-smoke",
            prompt,
        ]
        result = command_result(command, timeout_seconds=timeout_seconds)

    stdout = result["stdout"]
    stderr = result["stderr"]
    session_id = parse_session_id(stdout)
    readback = read_opencode_session(session_id, db_path) if session_id else {"status": "missing_session_id"}
...
def main() -> int:
    ...
    for role in roles:
        ...
        if harness != "opencode":
            report["model_check"] = {"status": "unsupported", "reason": "only opencode checks are implemented"}
            if args.require_models or args.smoke:
                failures.append(f"{role} harness is not supported by check_goal_config.py: {harness}")
        ...
        if args.smoke:
            ...
            if not isinstance(smoke, dict):
                ...
                smoke_report, smoke_failures = run_opencode_smoke(
                    role,
                    model,
                    smoke,
                    opencode_path=opencode_path,
