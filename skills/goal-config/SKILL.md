...
- Treat missing binaries, missing models, missing assistant output, auth/API errors, or smoke text mismatches as blocked evidence. The checker preserves the actionable provider/status/message/count fields; full raw provider payloads are intentionally not retained.
