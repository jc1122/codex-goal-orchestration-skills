...
- For `codex` role models, the config may use bare provider-implied model ids such as `--role-model worker_codex_spark:codex:openai/gpt-5.3-codex-spark`; it records the provider separately while passing the provider-free model id to the CLI. For `opencode-bridge` role models, use `--role-model ROLE:opencode-bridge:deepseek/deepseek-v4-flash` (or `deepseek-v4-pro`).
- For custom harnesses, pass `--harness-spec /abs/spec.json` or inline JSON. The spec should include `name`, `kind`, `command`, and smoke invocation; include `run_args`/`run_readback` when the launchers need explicit runtime metadata (custom harnesses and non-default routes usually do, while bundled defaults do not).
...
- Run `check_goal_config.py --config /abs/goal.config.json --require-models --stdout summary --output /abs/goal-config-check.json --state-output /abs/goal-config-state.json` before trusting configured model routes.
...
- Do not record USD/pricing fields. Do not inspect provider credentials or unrelated provider state.
