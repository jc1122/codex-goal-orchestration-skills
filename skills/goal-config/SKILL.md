...
- For discovery/use-all-available, run `check_goal_config.py --discover-profile mixed-fast --smoke --stdout summary --output /abs/goal-config-discovery.json --state-output /abs/goal-config-state.json`, inspect accepted/rejected routes, then create the final config with `create_goal_config.py --from-discovery /abs/goal-config-discovery.json --mapping auto`.
...
- Run `check_goal_config.py --smoke --stdout summary` when the user requests harness validation, debug telemetry, a new provider/model profile, or a discovery-style "use all available" profile.
