...
Prefer native branch-agent delegation whenever the runtime surface provides it. Before launching a selected branch, render and preserve a delegation report, for example `render_branch_worktree_commands.py --branch Bxx --delegation-report /abs/bundle/branches/Bxx.delegation.json`. The report records `preferred_delegation=native_agent`, the selected native or CLI mode, native availability provenance, the CLI branch-control Codex model, any CLI fallback reason, and a `scheduler_launch_command`. Main must run that scheduler command immediately before starting the native branch agent or CLI branch-control process. Use CLI worktree commands only as an explicit fallback when native delegation is unavailable or the user/runtime explicitly requests CLI mode; the default CLI branch-control model is `gpt-5.4-mini`.
...
Main must not launch Lite before prompt audit to pre-screen prompts. Lite launchers delegate the `ds-flash-max` bridge route (deepseek-v4-flash `--variant max`) in read-only `plan` mode through `opencode_worker.py delegate`, using the bridge control-script path and control version captured at packet creation, and write `advice.json` plus `telemetry.json`. The launcher and validator rehash every input, `task.md`, and `prompt.md`, and re-resolve the bridge control script; stale inputs, prompt/task drift, bridge control-script drift, or missing telemetry make the advice invalid or blocked. Validate advice with `scripts/validate_lite_advice.py` before using it. If Lite is unavailable, quota-limited, blocked, invalid, stale, or contradicted by branch artifacts, ignore it and continue with the normal status validation path unless the user explicitly required Lite. Record every used or ignored Lite packet in `main.status.json`; record `lite_advice: []` only when no relevant main Lite packet exists.
