...
`lite_advice` must be present, even when empty. Any recorded main Lite packet must point to existing manifest-owned `lite/<packet_id>/advice.json` and `lite/<packet_id>/input-files.json`, match source hashes exactly, and have `validation_status`/`validation_defects` matching actual `validate_lite_advice.py` output. Any relevant main Lite packet directory under manifest-owned `lite/` must be recorded, so an empty `lite_advice` array is valid only when no main Lite packet exists.
