...
def _validate_telemetry_header(
    defects: list[str], data: dict, path: str, *, packet_id: str | None, role: str | None
) -> None:
...
def _validate_telemetry_attempt(
    defects: list[str],
    attempt: dict,
    item_path: str,
    *,
    allowed_alias_set: set[str],
    called_aliases: list[str],
...
def _validate_telemetry_totals(
    defects: list[str], data: dict, path: str, *, attempts_len: int, called_count: int
) -> None:
...
def validate_telemetry_artifact(
    ...
    *,
    ...
    _validate_telemetry_header(defects, data, path, packet_id=packet_id, role=role)
    ...
    for index, item in enumerate(attempts):
        ...
        _validate_telemetry_attempt(
            defects,
            attempt,
            item_path,
            allowed_alias_set=allowed_alias_set,
            called_aliases=called_aliases,
...
class LedgerState(NamedTuple):
    """Mutable replay state plus the immutable manifest parameters.

    The set/dict fields are mutated in place; scalars that are reassigned
    (max_observed, refill_required) are threaded through the per-event
    transition helpers instead of living here.
    """
...
def _new_ledger_state(
    *,
    ...
    return LedgerState(
        ...
        blocked_excuses=set(),
        under_capacity_excuses=set(),
    )


def _relaunch_allowed(state: LedgerState, item_id: str) -> bool:
    if state.scheduler_kind == "branch-worker-pool":
        return True
    if state.scheduler_kind == "main-branch-pool":
        return state.blocked_reason_codes.get(item_id) in STALE_RELAUNCH_REASON_CODES
    return False


def _relaunch_candidate(state: LedgerState, item_id: str) -> bool:
    return (
        item_id in state.launched
        and item_id in state.closed
        and state.finished_status.get(item_id) != "pass"
        and _relaunch_allowed(state, item_id)
    )


def _eligible_ids(state: LedgerState) -> list[str]:
    eligible = []
    for item_id in state.expected_ids:
        if item_id in state.active:
            continue
        if item_id in state.launched and not _relaunch_candidate(state, item_id):
            continue
        deps = state.dependencies.get(item_id, [])
        if all(dep in state.closed and state.finished_status.get(dep) == "pass" for dep in deps):
            eligible.append(item_id)
    return eligible


def _failed_dependency_ids(state: LedgerState, item_id: str) -> list[str]:
    failed = []
...
def _unexcused_eligible(state: LedgerState) -> list[str]:
    excused = state.deferred_excuses | state.blocked_excuses | state.under_capacity_excuses
    return [
        item_id
        for item_id in _eligible_ids(state)
        if item_id not in excused and not _relaunch_candidate(state, item_id)
    ]
...
def _validate_ledger_header(
    ...
    *,
    ...
    allowed_manifest_sha256s: set[str] | None,
) -> list:
...
def _validate_event_envelope(defects: list[str], event: dict, event_path: str, *, index: int) -> str | None:
    """Validate seq/timestamp/runtime_ref/event name. Returns the event name or None if unsupported."""
    seq = event.get("seq")
    ...
    _event_reason_code(defects, event, event_path, required=event_name in SCHEDULER_REASON_EVENTS)
    return event_name


def _validate_event_manifest_stamp(
    defects: list[str],
    event: dict,
    event_path: str,
    *,
    manifest_path: Path | None,
    event_manifest_allowed: set[str],
) -> None:
...
def _validate_idle_capacity_guard(
    defects: list[str],
    state: LedgerState,
    event: dict,
    event_name: str,
    event_path: str,
    *,
    before_unexcused: list[str],
    refill_required: bool,
) -> None:
    if not (before_unexcused and len(state.active) < state.capacity):
        return
...
def _handle_ready_event(
    defects: list[str], state: LedgerState, event_id: str, event_path: str, *, before_eligible: list[str]
) -> None:
    if event_id in state.ready_seen:
    ...
    state.ready_seen.add(str(event_id))


def _handle_refill_event(
    defects: list[str], state: LedgerState, event: dict, event_path: str, *, before_eligible: list[str]
) -> None:
    eligible_values = event.get("eligible_ids")
    ...
    for value_index, value in enumerate(eligible_values):
        repair_relaunch_eligible = bool(isinstance(value, str) and _relaunch_candidate(state, value))
    ...
    if len(state.active) >= state.capacity:
        defect(defects, event_path, "refill requires free capacity")


def _handle_launch_event(
    defects: list[str],
    state: LedgerState,
    event_id: str,
    event_path: str,
    *,
    before_eligible: list[str],
    max_observed: int,
) -> int:
    repair_relaunch = bool(event_id and _relaunch_candidate(state, str(event_id)))
    if event_id in state.launched and not repair_relaunch:
    ...
    if event_id and event_id not in before_eligible and not repair_relaunch:
        failed_deps = _failed_dependency_ids(state, str(event_id))
        if failed_deps:
            defect(
                defects,
                f"{event_path}.id",
    ...
    if event_id:
        state.launched.add(str(event_id))
        state.active.add(str(event_id))
        if repair_relaunch:
            state.finished.discard(str(event_id))
            state.closed.discard(str(event_id))
            state.finished_status.pop(str(event_id), None)
        state.under_capacity.pop(str(event_id), None)
        state.deferred.pop(str(event_id), None)
        state.blocked.pop(str(event_id), None)
        state.under_capacity_excuses.discard(str(event_id))
        state.deferred_excuses.discard(str(event_id))
        state.blocked_excuses.discard(str(event_id))
        max_observed = max(max_observed, len(state.active))
    return max_observed


def _handle_finish_event(defects: list[str], state: LedgerState, event: dict, event_id: str, event_path: str) -> None:
    if event_id and event_id not in state.active:
    ...
    if event_id:
        ...
        if isinstance(status, str):
            state.finished_status[str(event_id)] = status


def _handle_close_event(
    defects: list[str], state: LedgerState, event_id: str, event_path: str, *, refill_required: bool
) -> bool:
    if event_id and event_id not in state.active:
        defect(defects, f"{event_path}.id", "cannot close an item that is not active")
    if event_id and event_id not in state.finished:
        defect(defects, f"{event_path}.id", "cannot close an item before finish")
    if event_id in state.closed:
        defect(defects, f"{event_path}.id", "duplicates close event")
    if event_id:
        state.active.discard(str(event_id))
        state.closed.add(str(event_id))
        if state.finished_status.get(str(event_id)) != "pass":
            state.under_capacity_excuses.discard(str(event_id))
            state.deferred_excuses.discard(str(event_id))
            state.blocked_excuses.discard(str(event_id))
    if _unexcused_eligible(state) and len(state.active) < state.capacity:
        return True
    return refill_required


def _handle_defer_event(defects: list[str], state: LedgerState, event: dict, event_id: str, event_path: str) -> None:
    reason = _event_reason(defects, event, event_path)
    reason_code = _event_reason_code(defects, event, event_path, required=True)
    if event_id:
        state.deferred[str(event_id)] = reason
        state.deferred_reason_codes[str(event_id)] = reason_code
        state.deferred_excuses.add(str(event_id))


def _handle_blocked_event(defects: list[str], state: LedgerState, event: dict, event_id: str, event_path: str) -> None:
    reason = _event_reason(defects, event, event_path)
    reason_code = _event_reason_code(defects, event, event_path, required=True)
    if event_id:
...
def _handle_under_capacity_event(
    ...
    for value_index, value in enumerate(eligible_values):
        if not isinstance(value, str) or value not in before_eligible:
            ...
            continue
        state.under_capacity[value] = reason
        state.under_capacity_reason_codes[value] = reason_code
        state.under_capacity_excuses.add(value)


def _validate_ledger_final_state(
    defects: list[str], state: LedgerState, path: str, *, require_all_launched: bool
) -> None:
    final_unexcused = _unexcused_eligible(state)
    if final_unexcused and len(state.active) < state.capacity:
        defect(
            defects,
            path,
    ...
    for item_id in _ordered_subset(state.launched - state.finished, state.expected_ids):
        defect(defects, path, f"launched item is missing a finish event: {item_id}")
    for item_id in _ordered_subset(state.launched - state.closed, state.expected_ids):
    ...
    if state.active:
        defect(
            ...
            + ", ".join(_ordered_subset(state.active, state.expected_ids)),
        )
    missing_launches = [item_id for item_id in state.expected_ids if item_id not in state.launched]
    ...
    for item_id in missing_launches:
        failed_deps = _failed_dependency_ids(state, item_id)
        if failed_deps:
            reason_code = (
                state.blocked_reason_codes.get(item_id)
                or state.deferred_reason_codes.get(item_id)
                or state.under_capacity_reason_codes.get(item_id)
            )
        ...
        if item_id not in state.deferred and item_id not in state.blocked and item_id not in state.under_capacity:
            defect(
                defects,
                path,
...
def _ledger_summary(state: LedgerState, max_observed: int) -> dict[str, object]:
    return {
        "launched": _ordered_subset(state.launched, state.expected_ids),
        "finished": _ordered_subset(state.finished, state.expected_ids),
        "active": _ordered_subset(state.active, state.expected_ids),
        "blocked": _ordered_subset(set(state.blocked), state.expected_ids),
        "deferred": _ordered_subset(set(state.deferred) | set(state.under_capacity), state.expected_ids),
        "finished_status": {
            item_id: state.finished_status[item_id]
            for item_id in state.expected_ids
            if item_id in state.finished_status
        },
...
def validate_scheduler_ledger(
    ...
    *,
    ...
    events = _validate_ledger_header(
        defects,
        root,
        path,
        scheduler_kind=scheduler_kind,
    ...
    for index, raw_event in enumerate(events):
        ...
        event_name = _validate_event_envelope(defects, event, event_path, index=index)
        if event_name is None:
            continue

        before_eligible = _eligible_ids(state)
        before_unexcused = _unexcused_eligible(state)

        _validate_event_manifest_stamp(
            defects,
            event,
            event_path,
            manifest_path=manifest_path,
            event_manifest_allowed=event_manifest_allowed,
        )
        _validate_idle_capacity_guard(
            defects,
            state,
            event,
            event_name,
            event_path,
            before_unexcused=before_unexcused,
        ...
        if event_name not in {"under_capacity", "refill"}:
            ...
            if event_id and event_id not in state.expected_set:
                defect(defects, f"{event_path}.id", "is not declared in the manifest scheduler item set")

        if event_name == "ready":
            _handle_ready_event(defects, state, event_id, event_path, before_eligible=before_eligible)
            continue
        if event_name == "refill":
            _handle_refill_event(defects, state, event, event_path, before_eligible=before_eligible)
            refill_required = False
            continue
        if event_name == "launch":
            max_observed = _handle_launch_event(
                defects, state, event_id, event_path, before_eligible=before_eligible, max_observed=max_observed
            )
            continue
        ...
        if event_name == "blocked":
            _handle_blocked_event(defects, state, event, event_id, event_path)
            continue
        if event_name == "under_capacity":
            _handle_under_capacity_event(defects, state, event, event_path, before_eligible=before_eligible)

    _validate_ledger_final_state(defects, state, path, require_all_launched=require_all_launched)
    return _ledger_summary(state, max_observed)
...
class LiteAdviceEntry(NamedTuple):
    """Parsed scalar fields of one runtime Lite advice entry."""
    ...
    disposition: object
    advice_path_value: str
    inputs_path_value: str
...
def _validate_lite_entry_fields(
    defects: list[str],
    data: dict,
    item_path: str,
    *,
    allowed_purposes: set[str],
    required_packet_prefix: str | None,
    reject_source_porcelain: bool,
    seen: set,
    reported_ids: set[str],
) -> LiteAdviceEntry:
    for key in LITE_ENTRY_REQUIRED_KEYS:
        if key not in data:
    ...
    require_string(defects, data.get("reason"), f"{item_path}.reason")
    return LiteAdviceEntry(
        packet_id=packet_id,
        purpose=purpose,
        avoids_action=avoids_action,
        expected_savings_reason=expected_savings_reason,
        disposition=disposition,
        advice_path_value=advice_path_value,
        inputs_path_value=inputs_path_value,
...
def _validate_lite_entry_paths(
    defects: list[str],
    entry: LiteAdviceEntry,
    item_path: str,
    advice_path: Path,
    inputs_path: Path,
    *,
    ...
    expected_dir = (manifest_path.parent / "lite" / entry.packet_id).resolve()
    ...
    if entry.validation_command and entry.validation_command != expected_command:
        defect(defects, f"{item_path}.validation_command", f"must be exactly: {expected_command}")


def _validate_lite_entry_against_inputs(
    defects: list[str], entry: LiteAdviceEntry, item_path: str, inputs_data: dict
) -> list:
    ...
    if entry.source_files != expected_min:
        defect(defects, f"{item_path}.source_files", "must match input-files.json source metadata exactly")
    if entry.avoids_action and entry.avoids_action != inputs_data.get("avoids_action"):
        defect(defects, f"{item_path}.avoids_action", "must match input-files.json avoids_action")
    if entry.expected_savings_reason and entry.expected_savings_reason != inputs_data.get("expected_savings_reason"):
...
def _validate_lite_entry_advice(
    defects: list[str],
    entry: LiteAdviceEntry,
    item_path: str,
    *,
...
def _validate_lite_advice_entry(
    defects: list[str],
    entry: LiteAdviceEntry,
    item_path: str,
    *,
    ...
    """Resolve and cross-check one entry's artifacts. Returns the (possibly loaded) lite_validator."""
    if not (
        entry.advice_path_value
        and entry.inputs_path_value
        and is_absolute_path(entry.advice_path_value)
        and is_absolute_path(entry.inputs_path_value)
    ):
        return lite_validator
    advice_path = Path(entry.advice_path_value).resolve()
    inputs_path = Path(entry.inputs_path_value).resolve()
    if entry.packet_id:
        _validate_lite_entry_paths(
            defects,
            entry,
            item_path,
            advice_path,
    ...
    if not isinstance(inputs_data, dict):
        ...
        return lite_validator
    expected_sources = _validate_lite_entry_against_inputs(defects, entry, item_path, inputs_data)
    ...
    if lite_validator is not None:
        _validate_lite_entry_advice(
            defects,
            entry,
            item_path,
            lite_validator=lite_validator,
...
def validate_runtime_lite_advice_entries(
    ...
    *,
    ...
    for index, item in enumerate(value):
        item_path = f"{path}[{index}]"
        data = require_object(defects, item, item_path)
        entry = _validate_lite_entry_fields(
            defects,
            data,
            item_path,
            allowed_purposes=allowed_purposes,
            required_packet_prefix=required_packet_prefix,
            reject_source_porcelain=reject_source_porcelain,
            seen=seen,
            reported_ids=reported_ids,
        )
        lite_validator = _validate_lite_advice_entry(
            defects,
            entry,
            item_path,
            manifest_path=manifest_path,
            script_dir=script_dir,
            validator_module_name=validator_module_name,
            lite_validator=lite_validator,
        )
