...
Parallelism is the default. Respect max_active_branch_agents from job.manifest.json; never exceed {MAX_ACTIVE_BRANCH_AGENTS}. Keep branch orchestrator slots saturated up to that cap: when a branch finishes and capacity is freed, launch the next eligible branch immediately. Defer a branch only while one of its manifest depends_on branch ids is incomplete; waves are scheduling/order groups, not implicit dependency barriers. Record scheduler events under schedulers/. Each branch entry must declare 1 to 4 worker packets and branch prompts require independent worker packets to launch as a rolling saturated pool up to the branch worker cap. After dispatch, wait for branch agents; do not poll active branch worktrees, worker packets, reviewer packets, process tables, or status files. Collect finished branch status/review artifacts and close finished branch orchestrator agents before launching replacements. A single-branch or otherwise serialized plan is valid only when job.manifest.json records serial_reasons or parallelization_rationale.
