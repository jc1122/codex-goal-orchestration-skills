...
- Preflight brief `source_attachments` may list repo-relative files or `{path,label,kind}` objects for large benchmark/spec inputs. Bundle creation stores file size and SHA-256 in the manifest and prompt instead of forcing all source text into the top-level goal.
