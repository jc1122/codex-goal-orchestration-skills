...
Worker packets are the default. Max active worker packets: {max_active_worker_packets}. Branch cap: 4. Never exceed {max_active_worker_packets} active workers here or 4 total. Launch ready workers as a rolling saturated pool with `render_worker_schedule.py --list-ready` before first launch and after each completion.
