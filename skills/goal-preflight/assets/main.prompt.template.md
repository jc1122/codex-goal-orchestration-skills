...
- If no branch completes after `orchestration_watchdog.main_no_completion_wait_limit` consecutive waits, inspect only native agent/process state. Do not block, close, replace, or relaunch a branch while a matching native process or launcher state is still live and reachable. Close only unreachable or stale active branches with `scheduler_tick.py --blocked/--close --reason-code stale_active|native_agent_unreachable|timeout`, then refill eligible capacity.
...
- Do not write a terminal/final response while any launched branch is still active unless you first materialize validator-visible non-pass evidence in `main.status.json`. An in-progress prose summary is not terminal evidence.
- Before any final response, run the finalize phase: `scheduler_tick.py --close-from-artifacts --validate-final`, `summarize_telemetry.py --bundle-dir {bundle_root_shell}`, `assemble_main_status.py --manifest {manifest_path_shell} --out {main_status_path_shell} --replace`, `validate_main_status.py --manifest {manifest_path_shell} --status {main_status_path_shell}`, and `reconcile_goal_run.py --manifest {manifest_path_shell} --write`.
- If finalization cannot pass, preserve the blocker as structured `partial` or `blocked` evidence in `main.status.json` and include exact validator defects; never end with only `TERMINAL_SUMMARY.md`, `mini-control.final.md`, or missing `main.status.json`.
