...
The runtime enforces skill availability bootstrap, absolute CLI entry paths, reproducible manifest paths, fixed prompt-audit and worker model fallback chains, prompt audit before branch work, parallel branch waves, and a hard limit of 4 active branch orchestrator agents. Preflight defaults to parallel decomposition, allows at most 5 waves of 4 branches, and requires a serial reason for single-branch bundles. Bundle prompt/status/review paths are relative to the bundle root; worktree paths are relative to the repository root.
